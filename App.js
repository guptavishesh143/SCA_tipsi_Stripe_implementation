...
const App = () => {
  ...
  const createPaymentMethod = async (token) => {
   ...
   //   console.log(paymentMethod);
    ...
    const cnfrmFunction = async (paymentMethodUid,client_secret) => {
    try {
    console.log(paymentMethodUid);
 ...
 const confirmPaymentIntented = () => {
  ...
  axios.get('https://us-central1-morrow-beta.cloudfunctions.net/paymentIntent')
  .then(res => {
    client_secret = res.data.client_secret;

    console.log(res.data.client_secret);
    cnfrmFunction(paymentMethodUid,client_secret);
    //  var obj = JSON.stringify(res)
    //console.log(obj);
   // cnfrmFunction(res.data.client_secret);
